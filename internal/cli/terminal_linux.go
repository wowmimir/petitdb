...
func enableRawMode(fd int) (interface{}, error) {
	...
	if _, _, err := syscall.Syscall6(syscall.SYS_IOCTL, uintptr(fd), syscall.TCSETS, uintptr(unsafe.Pointer(&termios)), 0, 0, 0); err != 0 {
		return nil, err
	}
	return orig, nil
}
...
func disableRawMode(fd int, state interface{}) {
	...
	syscall.Syscall6(syscall.SYS_IOCTL, uintptr(fd), syscall.TCSETS, uintptr(unsafe.Pointer(&orig)), 0, 0, 0)
}
